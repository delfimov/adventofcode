...
def strength(item):
    names_to_hex = {'A': 'E', 'K': 'D', 'Q': 'C', 'J': 'B', 'T': 'A'}
    hex_string = item
    ...
    p = int("0x" + hex_string, 16)
    c = sorted([[len(list(v)),k] for k,v in groupby(sorted(item))], key=cmp_to_key(lambda item1,item2: item2[0]-item1[0]))
